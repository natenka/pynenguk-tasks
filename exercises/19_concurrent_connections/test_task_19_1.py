...
def test_function_return_value():
    ...
    correct_reachable, correct_unreachable = sorted(correct_reachable), sorted(correct_unreachable)
    ...
    assert all(type(item) == list for item in return_value), "Функція повинна повертати кортеж зі списками усередині"

    return_reachable, return_unreachable = return_value
    return_reachable, return_unreachable = sorted(return_reachable), sorted(return_unreachable)
