...
def test_class_inheritance(first_router_from_devices_yaml):
    ...
    assert isinstance(ssh, CiscoIosSSH), "Клас MyNetmiko має успадкувати від CiscoIosSSH"
...
def test_errors_ignore_false(first_router_from_devices_yaml, command, error):
    ...
    ssh.disconnect()
    assert error in str(excinfo), "Метод send_config_commands повинен генерувати виключення, коли команда ignore_errors=False"
...
def test_errors_ignore_true(first_router_from_devices_yaml, command, error):
    ...
    ssh.disconnect()
    assert error in return_value, "Метод send_config_commands повинен повернути вивід з помилкою, коли ignore_errors=True"
