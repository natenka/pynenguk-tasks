...
def test_function_return_value(r1_test_connection, first_router_from_devices_yaml):
    ...
    correct_return_value = {first_router_from_devices_yaml["host"]: [dict(zip(re_table.header, line)) for line in result]}

    full_pth = os.path.join(os.getcwd(), "templates")
    return_value = task_21_5.send_and_parse_command_parallel([first_router_from_devices_yaml], "sh ip int br", templates_path=full_pth)
