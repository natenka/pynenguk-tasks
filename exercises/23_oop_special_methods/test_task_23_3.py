...
def test_method__add__(normalized_topology_example):
    ...
    top2 = task_23_3.Topology({("R1", "Eth0/4"): ("R7", "Eth0/0"), ("R1", "Eth0/6"): ("R9", "Eth0/0")})
    top2_size_before_add = len(top2.topology)

    check_attr_or_method(top1, method="__add__")
    top3 = top1 + top2
    assert isinstance(top3, task_23_3.Topology), "Метод __add__ має повернути новий екземпляр класу Topology"
